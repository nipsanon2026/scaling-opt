...
python -m pip install --no-deps "llmcompressor==0.10.0.1"
...
PRE_ROTATIONS="${PRE_ROTATIONS:-R1,R2}"
PRE_ROTATION_BLOCK_SIZE="${PRE_ROTATION_BLOCK_SIZE:-}"
PRE_ROTATION_TYPE="${PRE_ROTATION_TYPE:-hadamard}"
RUN_FINAL_ZEROSHOT="${RUN_FINAL_ZEROSHOT:-1}"
FINAL_ZEROSHOT_TASKS="${FINAL_ZEROSHOT_TASKS:-}"
...
CMD=(
  ...
  --pre_rotation_type "${PRE_ROTATION_TYPE}"
)
if [[ -n "${PRE_ROTATION_BLOCK_SIZE}" ]]; then
  CMD+=(--pre_rotation_block_size "${PRE_ROTATION_BLOCK_SIZE}")
fi
if [[ "${RUN_FINAL_ZEROSHOT}" == "1" ]]; then
  CMD+=(--run_final_zeroshot)
  if [[ -n "${FINAL_ZEROSHOT_TASKS}" ]]; then
    CMD+=(--final_zeroshot_tasks "${FINAL_ZEROSHOT_TASKS}")
  fi
else
  CMD+=(--no-run_final_zeroshot)
fi
